...
Use `terra_fallback_worker` only after diagnosed failure and confirmed worker stop.

Use unique immutable task and binding artifacts. Allow only one write-capable worker
at a time. Never switch workers because of elapsed time alone, and never remove a
